...
export default async function Movie({ details }) {
                ...
                <div className='text-center md:text-start'>
                    ...
                    {/* <div className='grid lg:grid-cols-2 xl:grid-cols-3 gap-4 mt-4'>
                        ...
                        <Details 
                            ...
                            text={details.production_countries.map((country) => (<li>{country.name}</li>))}
                        />
                        <Details 
                            title='Production Companies'
                            text={details.production_companies.map((company) => (<li>{company.name} ({company.origin_country})</li>))}
                        />
                        <Details 
                            title='Spoken Languages'
                            text={details.spoken_languages.map((language) => (<li>{language.english_name}</li>))}
                        />
                        <Details 
                            title='Official Site'
